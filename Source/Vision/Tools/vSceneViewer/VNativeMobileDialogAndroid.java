...
public class VNativeMobileDialogAndroid extends AlertDialog  {
	VNativeMobileDialogAndroid(Activity activity)
	{
		super(activity);
	}

	static native void SetDialogState(long nativePointer, int which);
	static native void SetDialogInstance(long nativePointer, VNativeMobileDialogAndroid dialog);
		...
		final String title, final String message, final String button0, final String button1, final String button2)
	{
		activity.runOnUiThread(new Runnable()
		{
			public void run()
			{
				...
				VNativeMobileDialogAndroid dialog = new VNativeMobileDialogAndroid(activity);
	...
	public static void ChangeText(Activity activity, final VNativeMobileDialogAndroid dialog, final String text)
	{
		activity.runOnUiThread(new Runnable()
		{
			public void run()
	...
	public static void CloseInstance(VNativeMobileDialogAndroid dialog)
	{
		dialog.dismiss();	
	}
}
